...
import { categoriesWithImg, categoriesWitoutImg } from './data/categories';
// import { actionHistory } from './data/actionHistory';

const handlers = [
  rest.get('/api/products?regionId=1&categoryId=1', (_, res, ctx) => {
    return res(ctx.status(200), ctx.json(productList));
  }),

  rest.get('/api/categories', (req, res, ctx) => {
    const includeImages = req.url.searchParams.get('includeImages');

    if (includeImages === 'true') {
      return res(ctx.status(200), ctx.json(categoriesWithImg));
    }

    return res(ctx.status(200), ctx.json(categoriesWitoutImg));
  }),

  /* 잠깐 참고할 코드들 ,, 나중에 삭제 예정입니다 */
